...
test_N=3;
test_n =0;
Newton_itr=zeros(test_N,1);Newton_time_used=Newton_itr;
LBFGS_itr=Newton_itr;LBFGS_time_used=Newton_itr;
Diftheta=Newton_itr;Difnrm=Newton_itr;
while test_n<test_N
    test_n=test_n+1;
    clearvars -except Newton_itr Newton_time_used LBFGS_itr LBFGS_time_used Diftheta Difnrm test_N test_n
    tau = .0e-8;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%% Constraints
    lh =  5;   % number of fixed off diagonal elements in each row
    ll =  5;    % number of off diagonal elements of lower bounds in each row
    lu =  5;    % number of off diagonal elements of upper bounds in each row
    ll = min(ll,n-1);
    lu = min(lu,n-1);
    lh = min(lh,n-1);
...
z1.u=zx(k_e+k_l+1:k);
    ...
    [X,z,F,lbfgs_itr,lbfgs_time_used]= CaliMatLbfgsCauchyLUNew(G,ConstrA,OPTIONS);
    [X1,z1,F,lbfgs_itr,lbfgs_time_used]= CaliMatLbfgsCauchyLU(G,ConstrA,OPTIONS);
    ...
    Newton_itr(test_n)      =newton_itr;
    Newton_time_used(test_n)=newton_time_used;
    LBFGS_itr(test_n)       =lbfgs_itr;
    LBFGS_time_used(test_n) =lbfgs_time_used;
    Diftheta(test_n)        =diftheta;
    Difnrm(test_n)         =difnrm;
    
    if test_n==test_N
        Test_Information=table((1:test_N)', Newton_itr,         Newton_time_used,  LBFGS_itr,         LBFGS_time_used, Diftheta,                   Difnrm,...
            'VariableNames',{'Test_Number','Newton_Iterations','Newton_Time_Used','LBFGS_Iterations','LBFGS_Iime_Used','Dual_Function_Difference','Norm_Gredient_Difference'})
