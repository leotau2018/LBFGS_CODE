function [X,z,info,itr,time_used] = CaliMatLbfgsCauchyLUNew(G,ConstrA,OPTIONS,z0)
% CaliMatLbfgsCauchyLUNew，利用了柯西点的信息，讲变量分成积极约束集和非积极约束集，然后返回x_k处迭代
%（不在柯西点迭代，跳过柯西点处梯度计算过程）搜索方向所使用的梯度来自于x_k
...
if disp
    fprintf('\n ******************************************************** \n')
    fprintf( '    The CauchyNew LBFGS method       ')
    fprintf('\n ******************************************************** \n')
...
%         [C1,C2,C3,B]=classify(zc_lu,gc_zlu, zb_l, w);    %%%对不等式部分的自由变量分类,原文题的等式约束所对应的对偶问题的变量皆为无约束
         g=[g_ze;g_zl;g_zu];
%          bb_lu=find(zc_lu<=10*eps);
%          F_lu=find(zc_lu>10*eps);
         [ds_e, ds_lu]=getDs_Free(S, Y, lr-1, start_index, gamma, M, k_e, k_lu, F_lu, bb_lu, zc_lu, zb_l,g);%get the search direction at cauchy poin
    else
