...
while test_n<test_N
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%% Constraints
    lh =  25;   % number of fixed off diagonal elements in each row
    ll =  25;    % number of off diagonal elements of lower bounds in each row
    lu =  25;    % number of off diagonal elements of upper bounds in each row
    ll = min(ll,n-1);
    lu = min(lu,n-1);
    lh = min(lh,n-1);
...
z1.u=zx(k_e+k_l+1:k);
    ...
    [X,z,F, newlbfgs_itr,newlbfgs_time_used]= CaliMatLbfgsCauchyLUNew_adapt(G,ConstrA,OPTIONS);
    ...
    if test_n==test_N
        Test_Information=table((1:test_N)', A_itr,         A_time_used,  B_itr,         B_time_used, Diftheta,                   Difnrm,...
            'VariableNames',{'Test_Number','Cauchynew_adapt_Iterations','Cauchynew_adapt_Time_Used','LBFGS2D_Iterations','LBFGS2D_Iime_Used','Dual_Function_Difference','Norm_Gredient_Difference'})
         %writetable(Test_Information,'Testresult\Test_Information_500_50_50_50(eb=1e-6).xls');
    end
